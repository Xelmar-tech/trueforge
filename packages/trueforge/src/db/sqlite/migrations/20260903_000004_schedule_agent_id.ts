    ...
    await db.transaction().execute(async trx => {
      ...
      await sql`
        INSERT INTO schedule_new (
          id, tenant_id, agent_id, agent_name, name, manifest, status,
          created_by_subject, created_at, updated_at
        )
        SELECT
          s.id, s.tenant_id, a.id, s.agent_name, s.name, s.manifest, s.status,
          s.created_by_subject, s.created_at, s.updated_at
        FROM schedule AS s
        JOIN agent AS a ON a.tenant_id = s.tenant_id AND a.name = s.agent_name
      `.execute(trx);
      await sql`DROP TABLE schedule`.execute(trx);
      await sql`ALTER TABLE schedule_new RENAME TO schedule`.execute(trx);
      await sql`CREATE INDEX schedule_agent_idx ON schedule (tenant_id, agent_name)`.execute(trx);
      await sql`CREATE UNIQUE INDEX schedule_name_uq ON schedule (tenant_id, agent_name, name)`.execute(trx);
      await sql`
    ...
    await db.transaction().execute(async trx => {
      ...
      await sql`
        INSERT INTO schedule_old (
          id, tenant_id, agent_name, name, manifest, status,
          created_by_subject, created_at, updated_at
        )
        SELECT
          id, tenant_id, agent_name, name, manifest, status,
