...
describe('TrueFoundryAgentStore', () => {
  ...
  it('createAgent rejects reserved names before calling ServiceFoundry', async () => {
    ...
    await expect(
      store.createAgent({ tenant_id: TENANT, name: 'tfg', manifest: manifest() }),
    ).rejects.toBeInstanceOf(AgentNameReservedError);
    await expect(
      store.createAgent({ tenant_id: TENANT, name: 'trueforge', manifest: manifest() }),
    ).rejects.toBeInstanceOf(AgentNameReservedError);
  ...
  it('createAgent rolls back SF when DB insert fails', async () => {
    ...
    await expect(store.createAgent({ tenant_id: TENANT, name: 'research', manifest: manifest() })).rejects.toThrow(
      'db failed',
    );
  ...
  it('updateAgent without manifest passes through to the inner store', async () => {
    const updated = record({ external_id: 'sf-1' });
    ...
    await expect(store.updateAgent({ tenant_id: TENANT, id: 'agent-1', external_id: 'sf-1' })).resolves.toBe(updated);
    expect(putRemoteAgent).not.toHaveBeenCalled();
    expect(updateAgent).toHaveBeenCalledWith({ tenant_id: TENANT, id: 'agent-1', external_id: 'sf-1' }, undefined);
  });
